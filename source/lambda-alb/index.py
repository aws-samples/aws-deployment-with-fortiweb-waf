...
def GetLicenseFileName(bucket_name, lic_prefix):
    ...
    licenses = []
    response = client.list_objects(Bucket=bucket_name, Prefix=lic_prefix)
...
def InitInsParam(event, count, lic_files):
    ...
    Tags = [
                {
                    ...
                    'Tags':[
                        ...
                        {
                            'Key':'Name',
                            'Value':Resource['CustomIdentifier']
                        }
                    ]
                }
          ]
    ...
    if Resource['FortiWebInstanceImage'].find('BYOL') != -1:
    ...
    if Resource['FortiWebHAMode'].find('active-passive') != -1:
        ...
        UserData += '\n\"HaTGHttps\":\"' +Resource['TargetGroupsHttp'] +'\",'
...
def create_ec2_instance(event):
    ...
    if event['ResourceProperties']['FortiWebInstanceImage'].find('BYOL') != -1:
